	...
	return {
				...
				onclick: function(item) {
					...
					if (lib.config.control_style == 'default') {
						...
						if (lib.config.extension_手杀ui__enable) {
							alert("检测到已打开手杀ui，请先关闭手杀ui并重启一次！！！");
						} else {
			...
			lib.skill._gzyinni = {
				...
				content: function() {
					...
					if (name == 'gameStart') {
						...
						gzyn1.style.bottom = "1.9px";
						gzyn1.style.left = "56.6%";
						gzyn1.style.height = "187.8px";
						gzyn1.style.width = "43.4%";
						gzyn1.style.zIndex = "75";
						player.appendChild(gzyn1)

						//未知势力图
						var yn = document.createElement("img");
						yn.src = decadeUIPath + "/assets/image/player_gz_cntry.png";
						yn.style.cssText = "pointer-events:none";
						yn.classList.add("player_gz_cntry");
						yn.style.display = "block";
						yn.style.position = "absolute";
						yn.style.top = "-8.3px";
						yn.style.left = "-4px";
						yn.style.height = "33.5px";
						yn.style.width = "33.5px";
						yn.style.zIndex = "75";
			...
			lib.skill._guozhan = {
				...
				filter: function(event, player, name) {
					if (name == 'gameStart') {
						...
						var s = event.player.getElementsByClassName("guozhan")
						return s.length > 0
					}
				},
				content: function() {
					var name = event.triggername;
					if (name == 'gameStart') {
						var isGz = player.getElementsByClassName("guozhan")
						var gz = document.createElement("img");
						gz.src = decadeUIPath + "/assets/image/unknown.png";
			...
			lib.skill.ghujia = {
				...
				intro: {
					...
					content: function(content, player) {
						return '已有' + "<font color='deepskyblue'>" + get.cnNumber(player.hujia) + "</font>" + '点护甲值';
					},
					markcount: "get.cnNumber(player.hujia)",
				},
			};
			//双将名字显示
			lib.skill._ShenYh1 = {
				...
				content: function() {
					var yh = document.createElement("img");
					group2 = lib.character[player.name2][1];
					yh.src = decadeUIPath + "/assets/image/" + group2 + "1.png";
					yh.style.display = "block";
					yh.style.position = "absolute";
					yh.style.top = "0px";
					yh.style.left = "65px";
			...
			lib.skill._longLevel = {
				...
				content: function() {
					...
					if (lib.config.extension_十周年UI_longLevel == 'sex') {
						var rarity = ['gold', 'yu', 'yan'].randomGet();
					};
						...
						Player.update = function(count, hp, hpMax, hujia) {
							if (!_status.video) {
								...
								if (this.hujia > 0) {
									...
									hujiat.innerText = this.hujia; //这一段，如果没有护甲图案，创建一个护甲图案，数值为护甲值              
								} else {
									//   this.unmarkSkill('ghujia');
									if (hujiat) hujiat.remove(); //如果没有护甲，那么删除护甲图案 
								}
            ...
            EventContent.equip = function () {
              ...
              if (!(lib.config['extension_EngEX_SSEquip'])) {
                if (lib.config.extension_十周年UI_equipStyle == 'off') {
                  ...
                  if (!(ele.length > 1)) {
                    var type = card.getAttribute("data-card-subype");
                    ele[0].style.backgroundImage = "url(" + decadeUIPath + "/assets/image/" + type + ".png)";
                    ele[0].classList.add("box");
                    var e = ele[0].children;
                    for (i = 0; i < e.length; i++) {
                      if (i == 0) {
                        e[i].textContent = e[i].textContent.split("").reverse().join("");
                        if (colour == 'heart' || colour == 'diamond') {
                          e[i].style.color = "#EF1806"; // 方块/红桃花色颜色显示
                        } else {
                          e[i].style.color = "#030303"; // 梅花/黑桃花色颜色显示
                        }
                        e[i].style.left = "81%";
                        e[i].style.fontSize = "11px"; // 装备字体大小
                      } else {
                        e[i].style.color = "#664739"; // 装备字体颜色显示
                        if (type == "equip4" || type == "equip3")
                          e[i].style.left = "29%";
                        else e[i].style.left = "16%";
                        e[i].style.fontSize = "12px"; // 装备字体大小
                      }
                      e[i].style.display = "block";
                      e[i].style.position = "absolute";
                      // e[i].style.textShadow = "1px 0 0 black, 0 1px 0 black, -1px 0 0 black, 0 -1px 0 black"; // 装备字体描边显示
                    }
                    eng.insertAfter(e[0], e[1]);
                    // ele[0].replaceChild(temp,e[1])
                    // var ppp = document.createElement("img");
                    // ppp.src = eng.url + "/images/q.jpeg";
                    // console.info(ppp)
                    // e[0].parentNode.insertBefore(ppp,e[0]);
                  }
                }
              }
								...
								init: function(card) {
									...
									if (lib.config['extension_EngEX_SSEquip']) {
										...
										if (!(ele.length > 1)) {
											...
											// if (subype == "equip4") {
											//     var pa = e[0].parentNode;
											//     console.info(pa.parentNode.style.left)
											//     // if (pa.parentNode.style.left != "61px") pa.parentNode.style.left = "61px";
											// }
										...
										if (this._transform && this.parentNode && this.parentNode.parentNode &&
											...
											if (bool) {
												...
												if (this.cf) {
													...
													var res = dcdAnim.stopSpine(this.cf);
												}
											}
										}
									}
							...
							player: {
								...
								stopDynamic: function(primary, deputy) {
									...
									if (primary && dynamic.primary) {
										...
										this.node.avatar.setBackground(this.name1, 'character');
									} else if (deputy && dynamic.deputy) {
										dynamic.stop(dynamic.deputy);
										dynamic.deputy = null;
										this.node.avatar2.setBackground(this.name2, 'character');
								...
								$dieAfter: function() {
									this.node.avatar.setBackground(this.name1, 'character');
									if (this.doubleAvatar) this.node.avatar2.setBackground(this.name2, 'character');
									var s = this.getElementsByClassName("skinYh");
						...
						click: {
							card: function(e) {
								...
								if (this.classList.contains('selected')) {
									...
									if (lib.config['extension_EngEX_cardSelect'] != 'off') {
										if (this.parentNode.classList == "handcards") {
											this.cf = eng.cardFace;
											this.cf.parent = this;
											this.cf.id = this.cardid;
											dcdAnim.playSpine(this.cf, this.cf)
										}
									}
		...
		*/
									...
									if (lib.config.cardtempname != 'off') {
										...
										if ((cards[i].name != cardname) || (cards[i].nature != cardnature)) {
											...
											if (cardname == 'huogong') {
												...
												tempname = get.translation(cardnature) + tempname;
												cards[i]._tempName.style.backgroundImage = 'url("' + lib.assetURL + "extension/十周年UI/image/vcard/huogong.png" + '")';
											}

											//火杀
											if (cardname == 'sha' && cardnature == 'fire') {
												tempname = get.translation(cardnature) + tempname;
												cards[i]._tempName.style.backgroundImage = 'url("' + lib.assetURL + "extension/十周年UI/image/vcard/huosha.png" + '")';
											}
											//雷杀
											if (cardname == 'sha' && cardnature == 'thunder') {
												tempname = get.translation(cardnature) + tempname;
												cards[i]._tempName.style.backgroundImage = 'url("' + lib.assetURL + "extension/十周年UI/image/vcard/leisha.png" + '")';
											}
											//冰杀
											if (cardname == 'sha' && cardnature == 'ice') {
												tempname = get.translation(cardnature) + tempname;
												cards[i]._tempName.style.backgroundImage = 'url("' + lib.assetURL + "extension/十周年UI/image/vcard/bingsha.png" + '")';
											}
											//刺杀
											if (cardname == 'sha' && cardnature == 'stab') {
												tempname = get.translation(cardnature) + tempname;
												cards[i]._tempName.style.backgroundImage = 'url("' + lib.assetURL + "extension/十周年UI/image/vcard/cisha.png" + '")';
											}
											//神杀
											if (cardname == 'sha' && cardnature == 'kami') {
												tempname = get.translation(cardnature) + tempname;
												cards[i]._tempName.style.backgroundImage = 'url("' + lib.assetURL + "extension/十周年UI/image/vcard/shensha.png" + '")';
											}



											//------------定义卡牌名称----------//
											if (cardnature != 'fire' && cardnature != 'thunder' && cardnature != 'ice' && cardnature != 'stab' && cardnature != 'kami') {
												cards[i]._tempName.dataset.name = cardname;
												tempname = get.translation(cardname);
												cards[i]._tempName.style.backgroundImage = 'url("' + lib.assetURL + "extension/十周年UI/image/vcard/" + cardname + ".png" + '")';
											}
											//--------------------------//
					...
					ui.create.me = function(hasme) {
						...
						if (lib.config['extension_EngEX_flowersToEveryone']) {
							eng.sendToEveryone();
						}
					};
							...
							gainSkill: {
								...
								gain: function(skill) {
									...
									if (!sender.skills.contains(skill) && lib.translate[skill]) {
										...
										/*      //获得技能变标记形式
         if(!info.intro)info.intro={        content:function(){
		...
		player.markSkill(skill);    */
										if (lib.config.extension_十周年UI_newDecadeStyle == 'on') {
											...
											sender.classList.add("flex")
											var sk = sender.skills[skill]
											if (!sk) {
												var span = document.createElement("span")
												span.setAttribute("skill", skill)
												span.innerHTML = lib.translate[skill]
												span.classList.add("box")
												sender.appendChild(span)
												var configs = lib.config['extension_EngEX_SSGainSkillsVisible'];
												if (configs == "huanhua") {
													sender.style.background = "#d2c5a08f"
												} else {
													sender.style.backgroundImage = "url(" + decadeUIPath + "/assets/image/update_bg.png)";
													sender.style.backgroundSize = "100% 100%";
										...
										if (lib.config.extension_十周年UI_newDecadeStyle == 'on') {
											...
											if (sender.children.length > 0) {
												for (var i = 0; i < sender.children.length; i++) {
													if (sender.children[i].getAttribute("skill") == skill) {
					...
					lib.element.content.addJudge = function() {
						...
						if (!lib.card[viewAs] || !lib.card[viewAs].effect) {
							...
							if (judgeText == "兵" || judgeText == "乐" || judgeText == "闪" || judgeText == "火" || judgeText == "洪" || judgeText == "浮") {
					...
					lib.element.player.$damagepop = function(num, nature, font, nobroadcast) {
						...
						if (this.damagepopups.length && !this.damagepopLocked) {
							...
							if (typeof node.popupNumber == 'number') {
								...
								if (popupNum < 0) {
									if (lib.config.extension_十周年UI_daojianfu == 'fu') {
										switch (node.nature) {
											case 'thunder':
												if (popupNum <= -2) {
									...
									} else if (lib.config.extension_十周年UI_daojianfu == 'jian') {
										switch (node.nature) {
											case 'thunder':
												if (popupNum <= -2) {
													decadeUI.animation.playSpine({
														name: 'SSXF_SX_guanjielei',
														action: 'play6'
													}, {
														scale: 0.6,
														parent: player
									...
									} else if (lib.config.extension_十周年UI_daojianfu == 'dao') {
										switch (node.nature) {
											case 'thunder':
												if (popupNum <= -2) {
													decadeUI.animation.playSpine({
							...
							case 'respond':
								...
								if ((lib.config.cardtempname != 'off') && ((card.name != cardname) || (card.nature != cardnature))) {
									...
									if (cardname == 'huogong') {
										...
										tempname = get.translation(cardnature) + tempname;
										if (lib.config.extension_十周年UI_cardPrettify == 'png') {
											card._tempName.style.backgroundImage = 'url("' + lib.assetURL + "extension/十周年UI/image/card/huogong.png" + '")';
										} else {
											card._tempName.style.backgroundImage = 'url("' + lib.assetURL + "extension/十周年UI/image/card/huogong.webp" + '")';
										}
									}
									//火杀
									if (cardname == 'sha' && cardnature == 'fire') {
										tempname = get.translation(cardnature) + tempname;
										if (lib.config.extension_十周年UI_cardPrettify == 'png') {
											card._tempName.style.backgroundImage = 'url("' + lib.assetURL + "extension/十周年UI/image/card/huosha.png" + '")';
										} else {
											card._tempName.style.backgroundImage = 'url("' + lib.assetURL + "extension/十周年UI/image/card/huosha.webp" + '")';
										}
									}
									//雷杀
									if (cardname == 'sha' && cardnature == 'thunder') {
										tempname = get.translation(cardnature) + tempname;
										if (lib.config.extension_十周年UI_cardPrettify == 'png') {
											card._tempName.style.backgroundImage = 'url("' + lib.assetURL + "extension/十周年UI/image/card/leisha.png" + '")';
										} else {
											card._tempName.style.backgroundImage = 'url("' + lib.assetURL + "extension/十周年UI/image/card/leisha.webp" + '")';
										}
									}
									//冰杀
									if (cardname == 'sha' && cardnature == 'ice') {
										tempname = get.translation(cardnature) + tempname;
										if (lib.config.extension_十周年UI_cardPrettify == 'png') {
											card._tempName.style.backgroundImage = 'url("' + lib.assetURL + "extension/十周年UI/image/card/bingsha.png" + '")';
										} else {
											card._tempName.style.backgroundImage = 'url("' + lib.assetURL + "extension/十周年UI/image/card/bingsha.webp" + '")';
										}
									}
									//刺杀
									if (cardname == 'sha' && cardnature == 'stab') {
										tempname = get.translation(cardnature) + tempname;
										if (lib.config.extension_十周年UI_cardPrettify == 'png') {
											card._tempName.style.backgroundImage = 'url("' + lib.assetURL + "extension/十周年UI/image/card/cisha.png" + '")';
										} else {
											card._tempName.style.backgroundImage = 'url("' + lib.assetURL + "extension/十周年UI/image/card/cisha.webp" + '")';
										}
									}
									//神杀
									if (cardname == 'sha' && cardnature == 'kami') {
										tempname = get.translation(cardnature) + tempname;
										if (lib.config.extension_十周年UI_cardPrettify == 'png') {
											card._tempName.style.backgroundImage = 'url("' + lib.assetURL + "extension/十周年UI/image/card/shensha.png" + '")';
										} else {
											card._tempName.style.backgroundImage = 'url("' + lib.assetURL + "extension/十周年UI/image/card/shensha.webp" + '")';
										}
									}
									...
									if (cardnature != 'fire' && cardnature != 'thunder' && cardnature != 'ice' && cardnature != 'stab' && cardnature != 'kami') {
										...
										tempname = get.translation(cardname);
										if (lib.config.extension_十周年UI_cardPrettify == 'png') {
											card._tempName.style.backgroundImage = 'url("' + lib.assetURL + "extension/十周年UI/image/card/" + cardname + ".png" + '")';
										} else {
											card._tempName.style.backgroundImage = 'url("' + lib.assetURL + "extension/十周年UI/image/card/" + cardname + ".webp" + '")';
										}
									}
									...
									//转标记
									card._tempName.innerHTML = '<div style="width:33px; height:31px; left:71px; top:5px; position:absolute; background-image: url(' + lib.assetURL + 'extension/十周年UI/image/vcard/zhuan.png);background-size: 100% 100%;"></div>';
									//--------------------------//
								}
								...
								if (duicfg.cardUseEffect && event.card && (!event.card.cards || event.card.cards.length == 1)) {
									...
									switch (name) {
										case 'caochuan':
											...
											break;
										case 'jiejia':
											decadeUI.animation.cap.playSpineTo(card, 'effect_jiejiaguitian');
											break;
										case 'juedou':
											decadeUI.animation.cap.playSpineTo(card, 'effect_juedou', {
												scale: 0.5
											});
											break;
										case 'sha':
			...
			decadeUI.BoundsCache = (function() {
				...
				if (config.qgqc_hujiayinxiao) {
					lib.skill._qgqc_hujiayinxiao = {
						trigger: {
							player: 'damageBegin4'
						},
						filter: function(event, player) {
						...
						frequent: true,
						content: function() {
							game.playAudio('../extension/十周年UI/audio/hujia.mp3');
						},
					}
				}
		...
		precontent: function() {
			...
			style.innerHTML = "@keyframes SKILLNAME{"
			for (var i = 1; i <= 20; i++) {
			...
			window.decadeModule = (function(decadeModule) {
				...
				decadeModule.init = function() {
					...
					//----卡牌标记类----//
					if (lib.config['extension_EngEX_cardSelect'] != 'off') {
						this.css(decadeUIPath + 'cardK.css')
					} else if (lib.config.extension_十周年UI_newDecadeStyle == 'on') {
						this.css(decadeUIPath + 'cardX.css');
					} else {
		...
		config: {
			...
			//护甲音效
			"qgqc_hujiayinxiao": {
				"name": "<b><font color=\"#FF6020\">美化•护甲音效",
				"intro": "开启后场上有护甲的角色失去护甲时播放音效",
				"init": false
			},
			foldCardMinWidth: {
				name: '折叠手牌最小宽度',
